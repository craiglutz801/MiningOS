/**
 * Default discovery prompts shown when "Default (all minerals)" is selected.
 * Kept in sync with mining_os/sql/005_discovery_prompts.sql so the UI always has text even if the API fails.
 */
export const DEFAULT_SYSTEM_INSTRUCTION = `You are an expert mining and mineral-resource analyst. Your job is to find specific US locations where a given mineral exists and has the LEAST RESISTANCE to making money—i.e., where we can plausibly own or claim and monetize the resource with minimal extra cost or proof.
...
export const DEFAULT_USER_PROMPT_TEMPLATE = `Find US locations where {{mineral}} is present and has the least resistance to monetization. Include ONLY states: {{states}}.
